...
def _load_first_existing(model_dir: Path, *names: str) -> Any:
    for name in names:
        path = model_dir / name
        if path.exists():
            return joblib.load(path)
    expected = ", ".join(names)
    raise FileNotFoundError(f"Missing model artifact. Expected one of: {expected}")
...
def save_artifacts(artifacts: dict[str, Any], model_dir: str | Path) -> None:
    ...
    if "model" in artifacts:
        artifacts["model"].save_model(model_dir / "xgboost_model.json")


def load_artifacts(model_dir: str | Path) -> dict[str, Any]:
    model_dir = Path(model_dir)
    model_json = model_dir / "xgboost_model.json"
    if model_json.exists():
        model = XGBRegressor()
        model.load_model(model_json)
    else:
        model = _load_first_existing(model_dir, "model.pkl", "xgboost_model.pkl")

    return {
        "model": model,
        "frequency_maps": _load_first_existing(model_dir, "frequency_maps.pkl"),
        "onehot_encoder": _load_first_existing(model_dir, "onehot_encoder.pkl"),
        "ordinal_encoder": _load_first_existing(model_dir, "ordinal_encoder.pkl", "venue_ordinal_encoder.pkl"),
        "scaler": _load_first_existing(model_dir, "scaler.pkl", "standard_scaler.pkl"),
        "clipping_bounds": _load_first_existing(model_dir, "clipping_bounds.pkl"),
        "feature_columns": _load_first_existing(model_dir, "feature_columns.pkl"),
        "metrics": _load_first_existing(model_dir, "metrics.pkl"),
        "feature_importance": _load_first_existing(model_dir, "feature_importance.pkl"),
    }
